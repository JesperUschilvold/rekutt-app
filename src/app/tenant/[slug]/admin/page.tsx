...
export default async function AdminHome({
  ...
  if (appUser.role !== "admin") redirect("/min-side");
    ...
    <div className="flex min-h-screen flex-col bg-bg">
      <header className="border-b border-border bg-surface">
        <div className="mx-auto flex h-16 max-w-[1080px] items-center justify-between px-6">
          <div>
            <p className="text-xs text-text-muted">{tenant.name} — Admin</p>
      ...
      <main className="mx-auto w-full max-w-[1080px] flex-1 px-6 py-12">
        ...
        <p className="mt-2 text-text-secondary">
          Dette er en placeholder — det faktiske dashboardet kommer i leveranse 07.
        </p>
        ...
        <div className="mt-8 rounded-lg border border-border bg-surface p-6">
          ...
          <dl className="mt-3 grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-sm">
            ...
            <dd className="text-text-primary">{tenant.name}</dd>
          </dl>
        </div>
